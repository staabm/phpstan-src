...
final class MutatingScope implements Scope
{
	...
	public function assignVariableFromVariable(string $targetVariable, string $sourceVariable, Type $type, Type $nativeType, ?TrinaryLogic $certainty = null): self
	{
		...
		$exprString = '$' . $targetVariable;
		$holder = new ConditionalExpressionHolder([
			'$'. $sourceVariable .'->version' =>  ExpressionTypeHolder::createYes(new Variable($targetParameterName), TypeCombinator::intersect($targetParameter->getType(), $parameterType->getTarget())),
		], new ExpressionTypeHolder(new Variable($parameter->getName()), $ifType, TrinaryLogic::createYes()));
		$conditionalTypes[$exprString][$holder->getKey()] = $holder;


		$exprString = '$' . $targetVariable;
		$scope = $scope->addConditionalExpressions($exprString, $holder);
		return $scope;
